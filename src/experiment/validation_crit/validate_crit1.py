...
def _count_correct(output, target, record):
	for point_idx in range(0,target['n_point']):
		...
		z_A = output[0,0, y_A, x_A] #zero-indexed
		z_B = output[0,0, y_B, x_B]

		assert(x_A != x_B or y_A != y_B)

		ground_truth = target['ordianl_relation'][point_idx]
...
def evaluate(data_loader, model, criterion, max_n_sample):
	...
	for i in range(0,n_iters):
		...
		total_validation_loss += batch_loss * relative_depth_target['n_point']
